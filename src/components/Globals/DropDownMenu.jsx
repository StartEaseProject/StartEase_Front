...
const DropDownMenu = ({
  ...
  useEffect(() => {
    if(isSpec){
      setText("Select your speciality")
    }
  }, [isSpec]);
  ...
  return (
    <div {...rest} className="text-thirdColor text-fs-300 relative">
      ...
      {(defaultTitle || (isdelete && text !== title)) && (
        <div
          onClick={() => {
            setText(defaultTitle ? defaultTitle : title);
          }}
          className="text-primaryColor hover:cursor-pointer text-fs-400 mt-4 inline-block"
        >
          Clear Room
        </div>
      )}
